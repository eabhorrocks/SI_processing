...
for uid = 1:nclus
    ...
    for cv = 1:2
        ...
        if PeakTime(uid,1)~=NewPeakLoc_up % Yes, take the 1st CV on purpose!
            if ~isnan(-(PeakTime(uid,1)-NewPeakLoc_up))
                ...
                ProjectedWaveform(:,uid,cv) = ProjectedWaveform(:,uid,cv);
                spikeMap(:,:,cv) = spikeMap(:,:,cv);
            end
            ...
            if PeakTime(uid,1)-NewPeakLoc_up<0
                ...
                if ~isnan(spikeWidth_up-(PeakTime(uid,1)-NewPeakLoc_up))
